...
public:
	...
	void PrintOffset(const char *tableName, const char *propName)
	{
		int offset = this->GetOffset(tableName, propName);
		cout << tableName << " --> " << propName << " [0x" << hex << offset << "]\n";
	}
...
extern CNetworkedVariableManager *netVars;