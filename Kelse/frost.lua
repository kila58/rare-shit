--[[
    ...
    Credits:
            C0BRA          - Prediction
            noPE           - Detours, lots of code
            Fr1kin         - Lots of code, lots of proof of concept code, lots of references, etc
            Dicks Johnson  - Help w/ Targetting sound
            Barney Stinson - PlayerHurt trigger.

    Version 1: Started 8/7/2014
    Version 1: Finished 8/8/2014

    Version 1.1: Started 8/11/2014
    Version 1.1: Finished 8/13/2014

    TODO:
        -Convert lots of shit over to metas
        -Learn how CUserCmd meta works (autofire is broken?)
        -Menu (maybe?)
]]
...
local function GetBoxPos( ent, box, group )
    ...
    local pos = __c.VMatrix['GetTranslation'](mat)
    local box = (min + max) * 0.5
     
    //box:Rotate(__c.VMatrix['GetAngles'](mat))
    // Keeping ^ in incase there's a reason for it, but so far, it's just clutter.
    pos = pos + box
...
local function cansee(ent)
    ...
    local tr = copy['util']['TraceLine'](trace)
...
local soundhasplayed = false
local playedonplayer = nil
function Frost.Target()
    ...
    if FrostInput['IsKeyDown'](KEY_B) then
        ...
        for k, v in next, copy['player']['GetAll']() do
            if(cansee(v) && IsValidEntity(v) && v != ply) then
                Frost['__targ'] = v
            if FrostTS(playedonplayer) == FrostTS(__c.Entity['EntIndex'](v)) then
                return // will spam sounds otherwise?
            end
            if soundhasplayed == false && v != nil then
                playedonplayer = __c.Entity['EntIndex'](v)
                //print(playedonplayer)
                //copy['surface']['PlaySound']('frostbot_locked.wav') //aw yeah baby we're locked, lets play a sound for good measure
                soundhasplayed = true
            elseif soundhasplayed == true && v != nil && FrostTS(playedonplayer) != FrostTS(__c.Entity['EntIndex'](v)) then
                //playedonplayer = nil
...
FrostHk( "player_hurt", "HitDetection", function(data)
local victim, attacker, newhp = data.userid, data.attacker, data.health
local ply = copy['LocalPlayer']()
    if attacker == __c.Player['UserID'](ply) then
        copy['surface']['PlaySound']('frostbot_hit.wav')
    end
end)
...
copy['surface']['CreateFont']('DefaultFixedOutline', {
        font = 'Tahoma',
        size = 11,
        weight = 0,
        shadow = true,
...
function Frost.RenderScreenspaceEffects() //XQZ from Herpes v3
...
FrostHk("CreateMove", "hii", Frost.Aimbot)
FrostHk("Think", "hii2", Frost.Target)
FrostHk("HUDPaint", "hii3", Frost.HUDPaint)
FrostHk("RenderScreenspaceEffects", "hii4", Frost.RenderScreenspaceEffects)